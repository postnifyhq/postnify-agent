...
const CLIENT_ID = process.env.POSTNIFY_OAUTH_CLIENT_ID!;
const CLIENT_SECRET = process.env.POSTNIFY_OAUTH_CLIENT_SECRET!;
const FRONTEND_URL = process.env.POSTNIFY_FRONTEND_URL || 'https://platform.postnify.com';
const API_URL = process.env.POSTNIFY_API_URL || 'https://platform.postnify.com/api';
const SERVER_URL = process.env.SERVER_URL || `http://localhost:${PORT}`;
const DATABASE_URL = process.env.DATABASE_URL!;

if (!CLIENT_ID || !CLIENT_SECRET) {
  console.error('POSTNIFY_OAUTH_CLIENT_ID and POSTNIFY_OAUTH_CLIENT_SECRET are required');
  process.exit(1);
}
...
function html(res: ServerResponse, status: number, body: string) {
  ...
  res.end(`<!DOCTYPE html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Postnify CLI Auth</title>
<style>*{margin:0;padding:0;box-sizing:border-box}body{font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,sans-serif;display:flex;align-items:center;justify-content:center;min-height:100vh;background:#0e0e0e;color:#fff}.card{background:#141414;border:1px solid #252525;border-radius:12px;padding:48px;max-width:480px;text-align:center}h2{margin-bottom:16px;font-size:24px}p{color:#a0a0a0;margin-bottom:24px;line-height:1.5}.code{font-family:monospace;font-size:36px;font-weight:bold;letter-spacing:4px;background:#0e1f20;border:1px solid #26B9C0;border-radius:8px;padding:16px 32px;display:inline-block;margin:16px 0;color:#26B9C0}.btn{display:inline-block;background:#26B9C0;color:#fff;text-decoration:none;padding:12px 32px;border-radius:8px;font-size:16px;font-weight:500;border:none;cursor:pointer}.btn:hover{background:#1fa3aa}.success{color:#26B9C0}.error{color:#ef4444}</style></head><body><div class="card">${body}</div></body></html>`);
}
...
function handleVerifyPage(req: IncomingMessage, res: ServerResponse) {
  ...
  html(res, 200, `
    <h2>Postnify CLI Authorization</h2>
...
async function start() {
  ...
  server.listen(PORT, () => {
    console.log(`Postnify CLI auth server running on ${SERVER_URL}`);
