# Postnify CLI - Advanced Examples

This directory contains examples demonstrating the full capabilities of the Postnify CLI, including posts with comments and multiple media.

## Understanding the Post Structure

The Postnify API supports a rich post structure:
...
```bash
postnify posts:create --json examples/post-with-comments.json
```
...
```bash
postnify posts:create --json examples/multi-platform-post.json
```
...
```bash
postnify posts:create --json examples/thread-post.json
```
...
for file in posts/*.json; do
  ...
  postnify posts:create --json "$file"
  sleep 2
done
```
...
postnify posts:create --json thread.json
```
...
2. **Invalid image path** - Ensure images are accessible URLs or uploaded to Postnify first
