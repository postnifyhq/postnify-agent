...
| `POSTNIFY_OAUTH_CLIENT_ID` | Yes | -                             | OAuth app client ID from Postnify |
| `POSTNIFY_OAUTH_CLIENT_SECRET` | Yes | -                             | OAuth app client secret from Postnify |
| `PORT` | No | `3111`                        | Server port |
| `SERVER_URL` | No | `http://localhost:{PORT}`     | Public URL of this server (used for generating links) |
| `POSTNIFY_FRONTEND_URL` | No | `https://platform.postnify.com` | Postnify frontend URL for OAuth redirects |
| `POSTNIFY_API_URL` | No | `https://api.postnify.com`      | Postnify API URL for token exchange |

## Setup

### 1. Create an OAuth app in Postnify

Go to Postnify Settings → Developer → OAuth Apps and create a new app. Set the callback URL to:
...
```bash
export DATABASE_URL="postgresql://user:password@localhost:5432/postnify_auth"
export POSTNIFY_OAUTH_CLIENT_ID="pca_xxx"
export POSTNIFY_OAUTH_CLIENT_SECRET="pcs_xxx"
export SERVER_URL="https://cli-auth.postnify.com"
```
...
| `POST` | `/device/verify` | Validates the user code and redirects to Postnify OAuth. |
| `GET` | `/device/callback` | Postnify redirects here after authorization. Exchanges the auth code for a token and stores it. |
