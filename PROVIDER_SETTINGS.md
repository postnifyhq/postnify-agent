...
The Postnify CLI supports platform-specific settings for each integration. Different platforms have different options and requirements.
...
```bash
postnify posts:create --json post-with-settings.json
```
...
```bash
postnify posts:create --json reddit-post.json
```
...
```bash
postnify posts:create --json youtube-video.json
```
